...
export default function SetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const message = await setPassword({ new_password: newPassword });
      setSuccess(message || 'Mot de passe défini avec succès !');
      setTimeout(() => {
        navigate('/dashboard');
      }, 2000);
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-green-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8">
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-bold text-gray-900">Définir le mot de passe</h2>
          <p className="text-gray-600 mt-2">Créez votre mot de passe pour finaliser votre compte</p>
        </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-gray-600 text-sm">
            Ce formulaire vous permet de créer un mot de passe pour votre compte après une connexion Google ou une réinitialisation.
          </p>
        </div>
      </div>
    </div>
